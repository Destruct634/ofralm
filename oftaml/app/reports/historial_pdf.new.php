...
<body>
    ...
    <?php if (count($historial) > 0): ?>
        <?php foreach ($historial as $entrada): ?>
            <div class="historia">
                ...
                <div class="historia-body">
                    ...
                    ?>
                        <hr>
                        <p><strong>Archivos Adjuntos (Imágenes):</strong></p>
                        <div class="archivos">
                            <?php foreach($archivos as $archivo): 
                                $extension = strtolower(pathinfo($archivo['nombre_original'], PATHINFO_EXTENSION));
                                
                                // --- CORRECCIÓN DE RUTA PARA WINDOWS/XAMPP ---
                                // 1. Construir la ruta relativa
                                $ruta_relativa = $archivo['ruta_archivo'] . $archivo['nombre_guardado'];
                                
                                // 2. Usar realpath para obtener la ruta absoluta limpia del sistema operativo
                                $ruta_absoluta_servidor = realpath(__DIR__ . '/../../public/' . $ruta_relativa);
                            ?>
                                <?php 
                                // Validamos que realpath haya funcionado y que el archivo exista
                                if ($ruta_absoluta_servidor && file_exists($ruta_absoluta_servidor) && in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])): 
                                ?>
                                ...
                                <?php elseif (in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                    <p style="color: red;">Error: Imagen no encontrada en el servidor.<br>
                                    <small>Ruta buscada: <?php echo __DIR__ . '/../../public/' . $ruta_relativa; ?></small></p>
                                <?php else: ?>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <p>No hay entradas en el historial clínico para este paciente.</p>
    <?php endif; ?>
</body>
</html>
<?php
...
// Permisos para leer imágenes locales
$mpdf->showImageErrors = true; 

$mpdf->WriteHTML($html);

$nombre_archivo = 'Historial-' . str_replace(' ', '_', $paciente['apellidos']) . '.pdf';
$mpdf->Output($nombre_archivo, 'I'); // Cambiado a 'I' para abrir en navegador primero
?>