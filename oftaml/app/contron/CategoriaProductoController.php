...
include_once '../../app/models/CategoriaProducto.php';
...
switch ($request_method) {
    case 'GET':
        if (!Auth::check('categorias_producto', 'ver')) { http_response_code(403); echo json_encode(["message" => "Acceso denegado."]); break; }
        $stmt = $categoria->leer();
        $categorias_arr = ["data" => []];
    ...
    case 'POST':
        if (!Auth::check('categorias_producto', 'crear')) { http_response_code(403); echo json_encode(["message" => "Acceso denegado."]); break; }
    ...
    case 'PUT':
        if (!Auth::check('categorias_producto', 'editar')) { http_response_code(403); echo json_encode(["message" => "Acceso denegado."]); break; }
        $data = json_decode(file_get_contents("php://input"), true);
        $id = $data['id'];
        unset($data['id']);
    ...
    case 'DELETE':
        if (!Auth::check('categorias_producto', 'borrar')) { http_response_code(403); echo json_encode(["message" => "Acceso denegado."]); break; }
        $data = json_decode(file_get_contents("php://input"), true);
